...
Use 'rosia trash list' to see available items before restoring.
...
Tips:
  • Use 'rosia trash list' to see available items
...
func runRestore(cmd *cobra.Command, args []string) error {
	...
	// Require item name argument
	if len(args) == 0 {
		logger.Error("Item name is required")
		return fmt.Errorf("item name is required (use 'rosia trash list' to see available items)")
	}
